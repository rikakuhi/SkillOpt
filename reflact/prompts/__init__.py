...
- **Generic** prompts live in ``reflact/prompts/*.md``
- **Env-specific** prompts live in ``reflact/envs/<env>/prompts/*.md``
...
def load_prompt(name: str, env: str | None = None) -> str:
    ...
    Lookup order:
      1. ``reflact/envs/{env}/prompts/{name}.md``  (if *env* given)
      2. ``reflact/prompts/{name}.md``              (generic default)
    ...
    searched = []
    if env is not None:
        searched.append(os.path.join("reflact/envs", env, "prompts", f"{name}.md"))
    searched.append(f"reflact/prompts/{name}.md")
